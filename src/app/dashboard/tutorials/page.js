...
export default function TutorialsListPage() {
    ...
    return (
        <div className="tutorials-list-page">
            <header className="page-header">
                <div className="header-left">
                    <div className="header-icon"><BookOpen size={24} /></div>
                    <div>
                        <h1>Mini Tutorials</h1>
                        <p>Kelola panduan dan tutorial step-by-step</p>
                    </div>
                </div>
                <Link href="/dashboard/tutorials/new" className="btn btn-primary">
                    <Plus size={18} /><span>Buat Tutorial</span>
                </Link>
            </header>

            <div className="stats-row">
                <div className="stat-card"><span className="stat-value">{stats.total}</span><span className="stat-label">Total</span></div>
                <div className="stat-card"><span className="stat-value published">{stats.published}</span><span className="stat-label">Published</span></div>
                <div className="stat-card"><span className="stat-value premium">{stats.premium}</span><span className="stat-label">Premium</span></div>
                <div className="stat-card"><span className="stat-value">{stats.totalSteps}</span><span className="stat-label">Langkah</span></div>
            </div>
            ...
            {tutorialsList.length === 0 ? (
                <div className="empty-state">
                    ...
                    <Link href="/dashboard/tutorials/new" className="btn btn-primary"><Plus size={18} /> Buat Tutorial</Link>
                </div>
            ) : (
                <div className="tutorials-grid">
                        ...
                        <div key={tutorial.id} className="tutorial-card">
                            <div className="card-header">
                                <div className="card-badges">
                                    <span className={`badge ${getDifficultyColor(tutorial.difficulty)}`}>{getDifficultyLabel(tutorial.difficulty)}</span>
                                    {tutorial.is_premium && (<span className="badge badge-premium"><Lock size={10} /> Premium</span>)}
                                </div>
                                <span className={`status-dot ${tutorial.published ? 'published' : 'draft'}`} title={tutorial.published ? 'Published' : 'Draft'} />
                            </div>
                            <div className="card-body">
                                <h3>{tutorial.title}</h3>
                                {tutorial.description && (<p className="card-desc">{tutorial.description}</p>)}
                            ...
                            <div className="card-meta">
                                {tutorial.category && (<span className="meta-item"><BookOpen size={14} />{tutorial.category}</span>)}
                                <span className="meta-item"><Layers size={14} />{tutorial.steps?.length || 0} langkah</span>
                                {tutorial.estimated_time && (<span className="meta-item"><Clock size={14} />{tutorial.estimated_time}</span>)}
                            </div>
                            <div className="card-actions">
                                <a href={`/tutorials/${tutorial.slug}`} target="_blank" rel="noopener noreferrer" className="btn-action" title="Preview"><Eye size={16} /></a>
                                <Link href={`/dashboard/tutorials/${tutorial.id}/edit`} className="btn-action edit" title="Edit"><Edit size={16} /></Link>
                                <button className="btn-action delete" onClick={() => setDeleteModal(tutorial)} title="Delete"><Trash2 size={16} /></button>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="modal-overlay" onClick={() => setDeleteModal(null)}>
                    <div className="modal delete-modal" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="modal-actions">
                            ...
                            <button className="btn btn-danger" onClick={() => handleDelete(deleteModal.id)}><Trash2 size={16} /> Hapus</button>
                        </div>
                    </div>
                </div>
            )}
