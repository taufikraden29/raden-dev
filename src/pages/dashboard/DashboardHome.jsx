...
const DashboardHome = () => {
    ...
    // Memoize recentPosts to avoid recalculation
    const recentPosts = useMemo(() => (posts || []).slice(0, 5), [posts]);

    // Memoize statCards - recreated only when stats changes
    // Add null safety for stats values
    ...
    return (
        <div className="dashboard-home">
            ...
            <section className="recent-section">
                <div className="section-header">
                    <h2>Recent Posts</h2>
                    <Link to="/dashboard/posts" className="btn btn-ghost btn-sm">
                        View All
                    </Link>
                </div>
                        ...
                        <table className="table">
                            ...
                            <tbody>
                                {recentPosts.map(post => (
                                    <tr key={post.id}>
                                        <td>
                                            <Link to={`/dashboard/posts/${post.id}/edit`} className="post-title-link">
                                                {post.title}
                                            </Link>
                                        </td>
                                        <td>
            ...
            <section className="quick-actions">
                ...
                <div className="actions-grid">
                    <Link to="/dashboard/posts/new" className="action-card card">
                        <Plus size={24} />
                        <span>Create New Post</span>
                    </Link>
                    <Link to="/dashboard/posts" className="action-card card">
                        <FileText size={24} />
                        <span>Manage Posts</span>
                    </Link>
                    <Link to="/" target="_blank" className="action-card card">
                        <Eye size={24} />
                        <span>View Blog</span>
                    </Link>
                </div>
